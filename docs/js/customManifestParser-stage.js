export class CustomManifestParser {
    ...
    Array.from(periods).forEach((period) => {
      ...
      Array.from(adaptationSets).forEach((adaptationSet) => {
        ...
        if (segmentTemplate) {
          ...
          const segmentDuration = parseInt(segmentTemplate.getAttribute('duration') || '0');
          const timeline = segmentTemplate.querySelector('SegmentTimeline');
          if (timeline) {
            const segments = [];
            Array.from(timeline.getElementsByTagName('S')).forEach(segment => {
              const t = parseInt(segment.getAttribute('t') || '0');
              const d = parseInt(segment.getAttribute('d') || '0');
              const r = parseInt(segment.getAttribute('r') || '0');
  ...
  async parseManifest(uri) {
    ...
    return { tracks, events, minimumUpdatePeriod, segmentTimelines, type: mpd.querySelector('MPD').getAttribute('type') };
  }
}
